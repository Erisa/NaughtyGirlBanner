module AntiRaid
  ...
  @bot.ready do |event|
    ...
    @config[:servers].each_key do |sid|
      ...
      if server.nil?
        ...
        print "[INFO] - #{server_name(event.bot.server(sid))} (#{sid})"
  ...
  @bot.message do |event|
    ...
    begin
      ...
      @bot.user(@config[:owner]).pm("Error occured trying to process join time for <@#{event.user.id}> in #{server_name(event.server)} (`#{event.server.id}`): ```ruby\n#{e}```\nJoined at: `#{event.user.on(event.server).joined_at}`")
      diff = 9_999_999_999
    end
    ...
    if @config[:servers][event.server.id][:blacklist] == true && diff < @config[:join_threshold] && @config[:blacklist].any? { |word| event.message.content.downcase.include?(word.downcase) }
      ...
      begin
        ...
        @bot.user(@config[:owner]).pm("Error occured trying to delete message by <@#{event.user.id}> from #{server_name(event.server)} (`#{event.server.id}`): ```ruby\n#{e}```")
        next
      end
  ...
  def self.crossban(user, origin_server = nil, invoker = nil, automated = true, reason = 'No reason provided.')
    ...
    @config[:servers].each_key do |sid|
      ...
      if automated
        full_reason = "[ Automated crossban by #{invoker.distinct} ]: Possible raid message in #{server_name(origin_server)}."
        ...
        full_reason = "[ Crossban by #{invoker.distinct} in #{server_name(origin_server)}]: #{reason}"
      end
      begin
        server.ban(user, 7, reason: full_reason)
      rescue StandardError => e
        puts "[ERROR^] #{e}"
        @bot.user(@config[:owner]).pm("Error occured trying to ban <@#{user.id}> from #{server_name(server)} (`#{sid}`): ```ruby\n#{e}```")
      end
    end
  end
